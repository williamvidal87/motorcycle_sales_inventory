...
@push('product-category-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openProductCategoryModal', () => {
            $('#productcategoryModal').modal('show')
        });
        
        window.livewire.on('closeProductCategoryModal', () => {
            $('#productcategoryModal').modal('hide')
        });
        
        
        
        window.livewire.on('openSwalDelete', (ProductCategoryID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', ProductCategoryID)
                }else{
                    window.Livewire.emit('refresh_productcategory_table')
                }
                });
        });
