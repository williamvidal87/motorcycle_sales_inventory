...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openInventoryModal', () => {
            $('#inventoryModal').modal('show')
        });
        
        window.livewire.on('closeInventoryModal', () => {
            $('#inventoryModal').modal('hide')
        });
        
        
        
        window.livewire.on('openSwalDelete', (InventoryID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', InventoryID)
                }else{
                    window.Livewire.emit('refresh_inventory_table')
                }
                });
        });
