...
@push('purchase-product-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openPurchaseProductModal', () => {
            $('#purchaseproductModal').modal('show')
        });
        
        window.livewire.on('closePurchaseProductModal', () => {
            $('#purchaseproductModal').modal('hide')
        });
        
        
        
        window.livewire.on('openSwalDelete', (PurchaseProductID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', PurchaseProductID)
                }else{
                    window.Livewire.emit('refresh_purchaseproduct_table')
                }
                });
        });
